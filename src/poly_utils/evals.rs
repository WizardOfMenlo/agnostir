    ...
    F: crate::FieldElement,
{
    ...
    pub fn evaluate(&self, point: &MultilinearPoint<F>) -> F {
        ...
        self.evals
            ...
            .fold(F::ZERO, |acc, (eval, (_, lag))| acc + (*eval * lag))
    }
...
fn eval_multilinear<F: crate::FieldElement>(evals: &[F], point: &[F]) -> F {
...
pub fn geometric_till<F: crate::FieldElement>(mut a: F, n: usize, x: &[F]) -> F {
    let k = x.len();
    assert!(n > 0 && n < (1 << k));
