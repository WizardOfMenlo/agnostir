...
where
    F: crate::FieldElement,
{
    pub(crate) fn lift<M: Embedding<Source = F>>(
        &self,
        embedding: &M,
    ) -> CoefficientList<M::Target> {
        CoefficientList {
    ...
    pub fn evaluate_at_extension<E>(&self, point: &MultilinearPoint<E>) -> E
    where
        F: PrimeField,
        E: ArkField<BasePrimeField = F>,
    {
        assert_eq!(self.num_variables, point.0.len());
...
impl<F> CoefficientList<F> {
    ...
    #[cfg_attr(feature = "tracing", instrument(skip_all, fields(size = self.coeffs.len())))]
    pub fn to_extension<E>(self) -> CoefficientList<E>
    where
        F: PrimeField,
        E: ArkField<BasePrimeField = F>,
    {
...
fn eval_multivariate<F: crate::FieldElement>(coeffs: &[F], point: &[F]) -> F {
...
fn eval_extension<F: PrimeField, E: ArkField<BasePrimeField = F>>(
    coeff: &[F],
    eval: &[E],
    scalar: E,
) -> E {
