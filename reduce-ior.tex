    \begin{tcolorbox}[breakable, enhanced, colback=white, boxrule=0.65pt]
        \textbf{Indexer input: } $(\ERA, \distIndex)$.\\
        \textbf{Indexer output: } The indexer runs the helper function $\CodeswitchOracles(\ERA)$ (see \cref{fig:codeswitch-oracles}) to generate the index oracles $(\word^{\ind}_1, \dotsc, \word^{\ind}_{\ell_{\ind}})$ required in the codeswitch steps, and outputs
        \begin{itemize}[nosep]
            \item $\Index' \define (\Code',\eps')$,
            \item $\IndexShort \define \bot$, and
            \item $\IndexOracle \define (\word^{\ind}_1, \dotsc, \word^{\ind}_{\ell_{\ind}})$.
        \end{itemize}
    \end{tcolorbox}
    \vspace{-13.72pt}
    \begin{tcolorbox}[breakable, enhanced, colback=white, boxrule=0.65pt]
    ...
    \begin{tcolorbox}[breakable, enhanced, colback=white, boxrule=0.65pt]
        ...
        \begin{itemize}[nolistsep]
            \item $\Prover$ computes $\word_i = \Enc{\CodeC'}(\msg_i)$ and $y^{\eval}_i \define \mH_i(\zB_2)$.
            \item $\Verifier$ samples $\zB^{\ood}_i \sample \Field^{\log k'}$.
        \end{itemize}
        \item $\Prover$ sends $\VectorOracleOf{\word}{\ell_{\msg}}$ and $[y^{\eval}_i]_{i \in [\ell_{\msg}]}$ to $\Verifier$.
        \item $\Verifier$ checks that $\sum_{\bB \in \Bits^{\log k - \log k'}} \EqPoly(\zB_1,\bB) \cdot y^{\eval}_i = \EvalValue$.
        \item $\Verifier$ sends $[\zB^{\ood}_i]_{i \in [\ell_{\msg}]}$ to $\Prover$.
        \item $\Prover$ sends $[y^{\ood}_i \define \mH_i(\zB^{\ood}_i)]_{i \in [\ell_{\msg}]}$ to $\Verifier$.
        \end{enumerate}
        
        \parhead{Generating codeswitch claims}
        \begin{enumerate}[nolistsep]
        \item $\Verifier$ samples $\numSpotChecks$ \ttushar{(@Anubhav what do we set $\numSpotChecks$ to?)} indices $\alpha_1, \dotsc, \alpha_{\numSpotChecks} \sample \Field^{\log \BlockLength}$ and sends them to $\Prover$.
        ...
        \label{item:IOR-codeswitch-claims}
        \begin{itemize}
            \item Both $\Prover$ and $\Verifier$ know certain integer parameters $\numIP, \numTIP, \ell_{\aux}$ (described in \cref{fig:codeswitch-claims}).
            \item Over the course of the subprotocol, $\Prover$ sends oracles $\VectorOracleOf{\word^{\aux}}{\ell_{\aux}}$ to $\Verifier$.
            \item For each $i \in [\numIP]$, $\Prover$ and $\Verifier$ agree on an oracle $\OracleOf{\word^{\IP}_i}$ chosen from the set $\left ( \VectorOracleOf{\word^{\ind}}{\ell_{\ind}}, \VectorOracleOf{\word}{\ell_{\msg}}, \VectorOracleOf{\word^{\aux}}{\ell_{\aux}} \right )$, as well as a vector $\vB^{\IP}_i \in \Field^{k'}$ and a scalar $\sigma^{\IP}_i \in \Field$, with the claim that
            \[
            ...
            \item For each $i \in [\numTIP]$, $\Prover$ and $\Verifier$ agree on two oracles $\OracleOf{\word^{\TIP}_{i,1}}, \OracleOf{\word^{\TIP}_{i,2}}$ chosen from the set $\left ( \VectorOracleOf{\word^{\ind}}{\ell_{\ind}}, \VectorOracleOf{\word}{\ell_{\msg}}, \VectorOracleOf{\word^{\aux}}{\ell_{\aux}} \right )$, as well as a vector $\vB^{\TIP}_i \in \Field^{k'}$ and a scalar $\sigma^{\TIP}_i \in \Field$, with the claim that
            \[
                ((\vB^{\TIP}_i, \sigma^{\TIP}_i), (\word^{\TIP}_{i,1}, \word^{\TIP}_{i,2}), (\msg^{\TIP}_{i,1}, \msg^{\TIP}_{i,2})) \in \TIP^{(\CodeC', \distIndex')} \enspace,
            \]
            where $\msg^{\TIP}_{i,1}, \msg^{\TIP}_{i,2} \in \Field^{k'}$ are witnesses known to $\Prover$.
        \end{itemize}
        \end{enumerate}
        ...
        \item Define 
        \label{item:IOR-sumcheck-polynomial}
        \begin{align*}
        ...
        \begin{align*}
            ...
            &[a^{\TIP}_{i,1}]_{i=1}^{\numTIP} \define [\mH^{\TIP}_{i,1}(\rB)]_{i=1}^{\numTIP}, \quad \text{and} \quad  [a^{\TIP}_{i,2}]_{i=1}^{\numTIP} \define [\mH^{\TIP}_{i,2}(\rB)]_{i=1}^{\numTIP} \enspace .
        \end{align*}
        ...
        \begin{align*}
            ...
            &\quad \; \; + \sum_{i=1}^{\numIP} \beta^{2\ell_{\msg} + i} \cdot a^{\IP}_i \cdot \vH^{\IP}_i(\rB) + \sum_{i=1}^{\numTIP} \beta^{2\ell_{\msg} + \numIP + i} \cdot a^{\TIP}_{i,1} \cdot a^{\TIP}_{i,2} \cdot \vH^{\IP}_i(\rB) \enspace .
        \end{align*}
        \end{enumerate}
        ...
        \begin{align*}
            ...
            &\quad \; \; \; + \sum_{i=1}^{\numTIP} \gamma^{2\ell_{\msg} + \numIP + i} \gamma^i \cdot a^{\TIP}_{i,1} + \sum_{i=1}^{\numTIP} \gamma^{2\ell_{\msg} + \numIP + \numTIP i} \gamma^i \cdot a^{\TIP}_{i,2} \enspace .
        \end{align*}
        ...
        \begin{align*}
            ...
            &\quad \; \; \; + \sum_{i=1}^{\numTIP} \gamma^{2\ell_{\msg} + \numIP + i} \gamma^i \cdot \OracleOf{\word^{\TIP}_{i,1}} + \sum_{i=1}^{\numTIP} \gamma^{2\ell_{\msg} + \numIP + \numTIP i} \gamma^i \cdot \OracleOf{\word^{\TIP}_{i,2}} \enspace ,
        \end{align*}
        and $\Prover$ defines $\msg'$ to be the underlying message obtained by computing an analogous linear combination of  $\msg_i, \msg^{\IP}_i, \msg^{\TIP}_{i,1}, \msg^{\TIP}_{i,2}$.
